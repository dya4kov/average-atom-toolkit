...
print("rho          T            E_inner_Tf   E_inner_sc          ")
sys.stdout.flush()
...
for i in range(TT.size):
	...
	T = TT[i]
	V = mass/(Avogadro*rho*aVol)
	results.append([i, rho, hartree*T, pool.apply_async(Inner_energy, args=(V, T))])
...
while ncompleted < ntotal:
	...
	for itask in range(len(results)):
		if results[itask][3].ready():
			...
			rho = results[itask][1]
			T   = results[itask][2]
			[E_inner_Tf, E_inner_sc] = results[itask][3].get()
			out = "%12.6e %12.6e %12.6e %12.6e" % (rho, T, E_inner_Tf, E_inner_sc)
			print(out)
