...
def Entropy(V, T):
	...
	return [Entropy_tf, Entropy_sc]

## Single process##
# print("rho = ",rho0)
# print("T            Entropy_Tf            Entropy_sc          ")
# file = open("entropy_sc.txt", "w")

# for T_cur in T: #np.array([1.0,5.0 ])
# 	V = mass/(Avogadro*rho*aVol)
# 	[Entropy_Tf,Entropy_sc] = Entropy(V,T_cur)
# 	out = "%12.6e %12.6e %12.6e" % (round(T_cur * hartree), Entropy_Tf, Entropy_sc)
# 	file.write(out + "\n")
# 	print(out)
...
print("rho          T            Entropy_Tf   Entropy_sc          ")
sys.stdout.flush()
...
for i in range(TT.size):
	...
	T = TT[i]
	V = mass/(Avogadro*rho*aVol)
	results.append([i, rho, hartree*T, pool.apply_async(Entropy, args=(V, T))])
...
while ncompleted < ntotal:
	...
	for itask in range(len(results)):
		if results[itask][3].ready():
			...
			rho = results[itask][1]
			T   = results[itask][2]
			[Entropy_tf, Entropy_sc] = results[itask][3].get()
			out = "%12.6e %12.6e %12.6e %12.6e" % (rho, T, Entropy_tf, Entropy_sc)
			print(out)
