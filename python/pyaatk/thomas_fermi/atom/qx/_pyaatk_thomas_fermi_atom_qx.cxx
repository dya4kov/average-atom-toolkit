...
PYBIND11_MODULE(_pyaatk_thomas_fermi_atom_qx, m) {

    auto& api = py::detail::npy_api::get();

    py::class_<aatk::TF::qx::Potential>(m, "Potential")
        .def(py::init([](){
            auto phi = new aatk::TF::qx::Potential();
            return phi;
        }))

        .def("__call__", [](aatk::TF::qx::Potential& phi, double x) -> double {
        ...
        .def("__call__", [api](aatk::TF::qx::Potential& phi, py::array_t<double> x) -> py::array {
            if (x.ndim() != 1) {
        ...
        .def("dx", [](aatk::TF::qx::Potential& phi, double x) -> double {
            bool correct_input = (x >= 0.0 && x <= 1.0);
            if (!correct_input) {
                throw std::runtime_error("Incorrect input: x should be between 0 and 1");
            }
        ...
        .def("dx", [api](aatk::TF::qx::Potential& phi, py::array_t<double> x) -> py::array {
            if (x.ndim() != 1) {
                throw std::runtime_error("Incorrect number of dimensions: should be 1D array");
            }
            // get c-array representation
        ...
        .def("setV", [](aatk::TF::qx::Potential& phi, double V) {
            phi.setV(V);
        })
        .def("setT", [](aatk::TF::qx::Potential& phi, double T) {
            phi.setT(T);
        })
        .def("setZ", [](aatk::TF::qx::Potential& phi, double Z) {
            phi.setZ(Z);
        })
        .def("setVTZ", [](aatk::TF::qx::Potential& phi, double V, double T, double Z) {
            phi.setVTZ(V,T,Z);
        })
        .def("setTolerance", [](aatk::TF::qx::Potential& phi, double tol){
