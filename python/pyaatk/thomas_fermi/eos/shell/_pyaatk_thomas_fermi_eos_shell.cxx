...
PYBIND11_MODULE(_pyaatk_thomas_fermi_eos_shell, m) {

    auto& api = py::detail::npy_api::get();

    py::class_<aatk::TF::shell::ChemicalPotential>(m, "ChemicalPotential")
        .def(py::init([](){
            auto M = new aatk::TF::shell::ChemicalPotential();
            return M;
        }))

        .def("__call__", [](aatk::TF::shell::ChemicalPotential& M, double V, double T) -> double {
            return M(V,T);
        })

        .def("__call__", [api](aatk::TF::shell::ChemicalPotential& M, double V, py::array_t<double> T) -> py::array {
            if (T.ndim() != 1) {
        ...
        .def("__call__", [api](aatk::TF::shell::ChemicalPotential& M, py::array_t<double> V, double T) -> py::array {
            if (V.ndim() != 1) {
        ...
        .def("__call__", [api](aatk::TF::shell::ChemicalPotential& M, py::array_t<double> V, py::array_t<double> T) -> py::array {
        ...
        .def("setNmax", [](aatk::TF::shell::ChemicalPotential& M, std::size_t nmax) {
            M.setNmax(nmax);
        })
        .def("setZ", [](aatk::TF::shell::ChemicalPotential& M, double Z) {
            M.setZ(Z);
        })
        .def("setTolerance", [](aatk::TF::shell::ChemicalPotential& M, double tol){
            M.setTolerance(tol);
        })
#ifdef ENABLE_MULTITHREADING
        .def("setThreadsLimit", [](aatk::TF::shell::ChemicalPotential& M, std::size_t Nthreads) {
