...
double ChemicalPotential::lastV1  = 1e-12;
double ChemicalPotential::lastT1  = 1e-12;
double ChemicalPotential::lastMu1 = 1e-12;

ChemicalPotential::ChemicalPotential() : 
    localTolerance(1e-6), localZ(1.0), threadsLimit(4)
{}

ChemicalPotential::ChemicalPotential(const ChemicalPotential& mu) {
    localTolerance = mu.localTolerance;
    localZ = mu.localZ;
...
void ChemicalPotential::setZ(const double& Z) { localZ = Z; }
...
void ChemicalPotential::setTolerance(const double& t) {
    localTolerance = std::max(t, bestTolerance);
}

double ChemicalPotential::operator()(const double& V, const double& T) {
    double result;
    double V1 = V*localZ;
    double T1 = T*std::pow(localZ, -4.0/3.0);
    if (equalLastV(V) && equalLastT(T)) {
        result = lastMu1;
    }
    else if (validBuf()) { // search in buffer table
        auto iv = vIndex(V);
        auto it = tIndex(T);
        if (iv >= 0 && it >= 0) {
            result = muBuf[iv*tBuf.size() + it]*std::pow(localZ, -4.0/3.0);
        }
    }
    else { // recalculate
        result = mu1(V1, T1, localTolerance);
    }
    lastV1 = V1;
    lastT1 = T1;
    lastMu1 = result;
    return result*std::pow(localZ, 4.0/3.0);
}
...
double* ChemicalPotential::operator()(
    ...
    prepareBuf(V, T, vsize, tsize);
    auto func = std::bind(&ChemicalPotential::mu, this, _1, _2, _3, _4);
    muBuf = std::move(evaluate(func, vBuf.data(), tBuf.data(), vsize, tsize));
...
std::vector<double> ChemicalPotential::operator()(
    ...
    prepareBuf(V.data(), T.data(), V.size(), T.size());
    auto func = std::bind(&ChemicalPotential::mu, this, _1, _2, _3, _4);
    muBuf = std::move(evaluate(func, vBuf.data(), tBuf.data(), V.size(), T.size()));
    std::vector<double> result(muBuf); // copy
    return result;
}
...
std::vector<double>& ChemicalPotential::evaluate(
    ...
    std::vector<double>* result = new std::vector<double>(vsize*tsize);
...
void ChemicalPotential::mu(
    ...
    double V1 = V*globalZ;
    double T1 = T*std::pow(globalZ, -4.0/3.0);
    double M1 = mu1(V1, T1, globalTolerance);
    result = M1*std::pow(globalZ, 4.0/3.0);
    finished = true;
}
...
bool ChemicalPotential::equalLastV(const double& V) {
    double V1 = V*localZ;
    return std::abs(V1 - lastV1) < bestTolerance;
}

bool ChemicalPotential::equalLastT(const double& T) {
    double T1 = T*std::pow(localZ, -4.0/3.0);
    return std::abs(T1 - lastT1) < bestTolerance;
}
...
void ChemicalPotential::prepareBuf(
    ...
    vBuf = std::move(sorted(V, vsize));
    tBuf = std::move(sorted(T, tsize));
...
std::vector<double>& ChemicalPotential::sorted(
    ...
    std::vector<double>* result = 
        new std::vector<double>(array, array + size);
