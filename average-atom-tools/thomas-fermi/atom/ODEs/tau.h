...
protected:
	struct RHSTau {
		...
		RHSTau() : V(1.0), T(1.0), mu(1.0), energyArg(1.0), lambdaArg(1.0) {
            rhs = std::bind(&RHSTau::rhsT, this, _1, _2, _3); eval_a();
        }

        void set_V (const double& _V)  { V = _V; eval_a(); }
        void set_T (const double& _T)  { T = _T; 
            rhs = T > 1e-10 
                    ? std::bind(&RHSTau::rhsT,  this, _1, _2, _3) 
                    : std::bind(&RHSTau::rhsT0, this, _1, _2, _3);
        }
	...
	private:
        ...
        void rhsT(const double& x, Array<dim>& y, Array<dim>& dydx) {
            ...
            dydx[2] = p > 0.0 ? -1.0/std::sqrt(p) : 0.0;
        }
        // rhs at T = 0
