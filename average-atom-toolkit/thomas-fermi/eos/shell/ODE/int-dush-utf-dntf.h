...
struct RHS_int_dush_utf_dntf {
	static const Dimension dim  = 5;

	RHS_int_dush_utf_dntf() : V(1.0), T(1.0), Z(1.0), mu(4.100577730112) {
        rhs = std::bind(&RHS_int_dush_utf_dntf::rhsT, this, _1, _2, _3); eval_a(); eval_r0();
    }

    ~RHS_int_dush_utf_dntf() { delete dnsh; }
    ...
    void set_T   (const double& _T)   { 
        ...
        T52 = T*T32;
        rhs = T > 1e-10 
                ? std::bind(&RHS_int_dush_utf_dntf::rhsT,  this, _1, _2, _3) 
                : std::bind(&RHS_int_dush_utf_dntf::rhsT0, this, _1, _2, _3);
    }
...
private:
    ...
    void rhsT(const double& x, Array<dim>& y, Array<dim>& dydx) {
        ...
        if (x > 0) {
        	...
        	dntf    = a*x*T12*FDmhalf(phi/(T*x2));
        	dydx[3] = dntf*y[2] + 2.0/x*dnsh->operator()(x);
            dydx[4] = -x2*y[2]*(phi + 1.0/r0)*dntf/a;
        }
        else {
            dydx[1] = 4.0/3.0*a*std::sqrt(phi)*phi;
            dntf    = 2.0*a*std::sqrt(phi);
            dydx[3] = dntf*y[2] - dydx[1]*Z43;
            dydx[4] = 0.0;
        }
    ...
    void rhsT0(const double& x, Array<dim>& y, Array<dim>& dydx) {
        ...
        double dntf = 2.0*a*std::sqrt(phi);
