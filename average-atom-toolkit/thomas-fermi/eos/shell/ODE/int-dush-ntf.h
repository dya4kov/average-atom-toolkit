...
struct RHS_int_dush_ntf {
	static const Dimension dim  = 5;

	RHS_int_dush_ntf() : V(1.0), T(1.0), Z(1.0), mu(4.100577730112) {
        rhs = std::bind(&RHS_int_dush_ntf::rhsT, this, _1, _2, _3); eval_a(); eval_r0();
    }

    ~RHS_int_dush_ntf() { delete dnsh; }
    ...
    void set_T   (const double& _T)   { 
        ...
        T52 = T*T32;
        rhs = T > 1e-10 
                ? std::bind(&RHS_int_dush_ntf::rhsT,  this, _1, _2, _3) 
                : std::bind(&RHS_int_dush_ntf::rhsT0, this, _1, _2, _3);
    }
...
private:
    ...
    void rhsT(const double& x, Array<dim>& y, Array<dim>& dydx) {
        ...
        if (x > 0) {
            ...
            dydx[4] = -x3*y[2]*T32*FDhalf(phi/(T*x2));
        }
        else {
