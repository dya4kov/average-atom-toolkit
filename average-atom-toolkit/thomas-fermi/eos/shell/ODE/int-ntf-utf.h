...
struct RHS_int_ntf_utf {
    static const Dimension dim    = 3;
    static const Dimension result = 2;

    RHS_int_ntf_utf() : V(1.0), T(1.0), mu(1.0), e(1.0) {
        rhs = std::bind(&RHS_int_ntf_utf::rhsT, this, _1, _2, _3); eval_a();
    }
    ...
    void set_T (const double& _T)  { 
        ...
        T52 = T*T32; 
        rhs = T > 1e-10 
                ? std::bind(&RHS_int_ntf_utf::rhsT,  this, _1, _2, _3) 
                : std::bind(&RHS_int_ntf_utf::rhsT0, this, _1, _2, _3);
    }
    ...
    void rhsT(const double& x, Array<dim>& y, Array<dim>& dydx) {
        ...
        if (x > 0.0) {
            ...
            if (phiBE <= 0.0) dydx[2] = 0.0;
                ...
                if (argY - argX > 25.0)
                    dydx[2] = -y[0]*x3*T32*FDhalf(argX);
                else
                    dydx[2] = -y[0]*x3*T32*FDhalfI(argX, argY);
            }
        }
        else {
    ...
    void rhsT0(const double& x, Array<dim>& y, Array<dim>& dydx) {
        ...
        if (x > 0.0) {
            if (phiBE <= 0) dydx[2] = 0.0;
                ...
                dydx[2] = -y[0]*std::pow(std::min(e, mu)*x2 + y[0], 1.5);
            }
        }
        else {
            dydx[2] = 0.0;
