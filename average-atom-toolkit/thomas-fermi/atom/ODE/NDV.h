...
struct RHSNDV {
    ...
    RHSNDV() : V(1.0), T(1.0), mu(1.0) { 
        rhs = std::bind(&RHSNDV::rhsT, this, _1, _2, _3); eval_a();
    }
    void set_V (const double& _V)  { V = _V; eval_a(); }
    void set_T (const double& _T)  { T = _T; 
        rhs = T > 1e-10 
                ? std::bind(&RHSNDV::rhsT,  this, _1, _2, _3) 
                : std::bind(&RHSNDV::rhsT0, this, _1, _2, _3);
    }
    ...
    void rhsT(const double& x, Array<dim>& y, Array<dim>& dydx) {
        ...
        if (x > 0) {
            ...
            dydx[3] = a*x*std::sqrt(T)*(
                         4.0/(3.0*V)*x2*T*FDhalf(phi/(T*x2)) + 
                                         FDMhalf(phi/(T*x2))*y[2]
                    );
        }
        else {
            dydx[1] = 4.0/3.0*a*std::sqrt(phi)*phi;
            dydx[3] = 2.0*a*std::sqrt(phi)*(
                      4.0/(9.0*V)*phi + y[2]
                );
        }
    }
    void rhsT0(const double& x, Array<dim>& y, Array<dim>& dydx) {
        ...
        dydx[1] = 4.0/3.0*a*std::sqrt(phi)*phi;
        dydx[2] = 2.0*x*y[3];
        dydx[3] = 2.0*a*std::sqrt(phi)*(4.0/(9.0*V)*phi + y[2]);
    }
