...
struct RHSdF {
    ...
    RHSdF() : V(1.0), T(1.0), mu(1.0) {
        rhs = std::bind(&RHSdF::rhsT, this, _1, _2, _3); eval_a();
    }
    void set_V (const double& _V)  { V = _V; eval_a(); } 
    void set_T (const double& _T)  { T = _T; 
        rhs = T > 1e-10 
                ? std::bind(&RHSdF::rhsT,  this, _1, _2, _3) 
                : std::bind(&RHSdF::rhsT0, this, _1, _2, _3);
    }
    void set_mu(const double& _mu) { mu = _mu; }

    double param() { return 2.0*V/(M_PI*M_PI*M_PI); }
